...
function Player() {
    ...
    tCharacter.checkCollision = function (monster) {
        ...
        tCharacter.checkCollision = function (bossMonster) {
            if (bossMonster.collidesWith(this)) {
                this.characterHP -= 30;
                console.log(this.characterHP + ' / ' + this.characterMHP)
                if (this.characterHP <= 0) {
                    game.stop()
...
function FireBall() {
    ...
    sFireball.checkCollision = function (monster) {
        if (monster.collidesWith(this)) {
            ...
            this.reset()
        } else if (bossMonster.collidesWith(this)) {
            bossMonster.reset()
            skillPoint++;
            killed++;
            console.log('Skill Point : ' + skillPoint)
...
function BossMonster() {
    ...
    var tBossMonster = new Sprite(game, "Boss_1", 288, 384);
    tBossMonster.loadAnimation(288, 384, 72, 96)
    tBossMonsterv.generateAnimationCycles()
    tBossMonster.setAnimationSpeed(500)

    tBossMonster.setSpeed(0);
    tBossMonster.wriggle = function () {
